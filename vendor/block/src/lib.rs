...
// upstream declares `enum Class { }` — an uninhabited type — and links an extern static
// of it, which is a future-incompat lint (error in a future rustc). A zero-sized opaque
// struct keeps the same ABI while being inhabited.
#[repr(C)]
struct Class {
    _priv: [u8; 0],
}
